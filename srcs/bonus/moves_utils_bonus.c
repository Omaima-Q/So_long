...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/12 16:47:15 by omaimaqaroo       #+#    #+#             */
/*   Updated: 2024/11/13 17:48:35 by omaimaqaroo      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	is_exit(t_game *game, int direction)
{
	if (game->map[game->x][game->y] == EXIT)
	{
		...
		{
			if (direction == LEFT)
				is_win(game, game->sprite.left, game->sprite.right, LEFT);
			if (direction == RIGHT)
				is_win(game, game->sprite.right, game->sprite.left, RIGHT);
			if (direction == DOWN)
				is_win(game, game->sprite.play, game->sprite.up, DOWN);
			if (direction == UP)
				is_win(game, game->sprite.up, game->sprite.play, UP);
		}
...
/*Checks if the location that we are on is a collectible. If so, changes the
location for the char '0' and reduces the number of collectibles missing,
